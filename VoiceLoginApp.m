...
function VoiceLoginApp
    ...
    % FIX: Add paths relative to ROOT
    ...
    function performLogin()
        ...
        statusLight.FontColor = [1 0.2 0.2];
        drawnow;
        ...
        statusLight.FontColor = [1 0.8 0];
        drawnow;

        audioData = getaudiodata(rec);
        tempFile = 'results/login_attempt.wav'; % Save to results
        audiowrite(tempFile, audioData, fs);

        try
            % FIX: Use the engine directly without CDing
            speakerName = identify_speaker_gmm(tempFile);
